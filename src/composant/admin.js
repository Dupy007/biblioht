...
class Admin extends Component{
    constructor(props) {
	  ...
	  postShowU = (show) => {
	   ...
	   fetchUsers = () => {
        fetch(this.baseURL+'/php/connect-admin.php')
        .then(response => {
    ...
    componentDidMount(){
	  ...
	  mySubmitHandler = (event) => {
		...
		const API_PATH =this.baseURL+"/php/connect-admin.php"
		const cc = this
		if ( mdp==='' || email==='') {
  ...
  render(){
	...
	let sess = this.state.users.map(({idAdmin,pseudoAdmin}, index) => {
		return idAdmin ? (
			sessionStorage.setItem('biblioht_prenom',pseudoAdmin),
			sessionStorage.setItem('biblioht_cpt','admin'),
			sessionStorage.setItem('biblioht_id',idAdmin),
			window.location = '/Livre'
